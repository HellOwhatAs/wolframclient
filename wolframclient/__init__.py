...
if get_ipython()!=None:
    ...
    def plot(expr,_type="SVG"):
        """_type=["SVG","JPEG","PNG"]"""
        if _type=="SVG":display.display_svg(section.evaluate(wl.ExportString(expr,_type)),raw=True)
        elif _type=="JPEG":display.display_jpeg(section.evaluate(wl.ExportByteArray(expr,_type)),raw=True)
        elif _type=="PNG":display.display_png(section.evaluate(wl.ExportByteArray(expr,_type)),raw=True)
